...
async function login() {
    try {
        ...
        try {
            ...
            console.debug(`Cookie: ${response.headers['set-cookie']}`)
        ...
        if (!userPass) {
            ...
            try {
                ...
                if (bodyArray[0] != 0) {
                    ...
                    console.info('login Not OK, start login first, then request');
                    await login();
...
async function getForecastData() {
    ...
    lat = '51.4337405';
    lon = '-2.8426622';
    dec = '25'; // angle of solar panels
    az = '130';  // azimuth of panels
    const objkwp = 1;

    kwp = objkwp.val / 1000;

    const urlProg = `${urlForecast + cmdForecast + lat}/${lon}/${dec}/${az}/${kwp}`;
    console.debug(`Inquiry data: ${urlProg}`);
    ...
    https.get(urlProg, res => {
        ...
        res.on('end', async () => {
            console.debug('RESPONSE ENDED: ' + JSON.stringify(res.result));

            try {
                const forecast = JSON.parse(Buffer.concat(data).toString());
                console.debug(`forecast: ${JSON.stringify(forecast)}`);

                if (forecast.message.type === 'success') {
                    const watthoursday = forecast.result;
                    console.debug(`Watt Hour Day = ${JSON.stringify(watthoursday)}`);
                    const watthourstoday = parseInt(forecast.result[new Date().toISOString().slice(0, 10)]);
                    console.debug(`Forcast for Today, ${new Date().toISOString().slice(0, 10)}: ${watthourstoday}`);
                    const tomorrow = new Date();
                    tomorrow.setDate(new Date().getDate() + 1);
                    const watthourstomorrow = parseInt(forecast.result[tomorrow.toISOString().slice(0, 10)]);
                    console.debug(`Forecast for tomorrow, ${tomorrow.toISOString().slice(0, 10)}: ${watthourstomorrow}`);

                    console.info('forecast.today', parseInt(watthourstoday, 10), true);
                    console.info('forecast.tomorrow', parseInt(watthourstomorrow, 10), true);
                    console.info('info.latitude', lat, true);
                    console.info('info.longitude', lon, true);
                    console.info('info.inclination', dec, true);
                    console.info('info.azimuth', az, true);
                } else {
                    console.warn('Forecast error, no forecast values values received');
                    console.info(`Answer type: ${forecast.message.type}`);
                    console.info(`Answer Text: ${forecast.message.text}`);
                    console.info(`Requests remaining (Limit:12): ${forecast.message.ratelimit.reamining}`);
                    console.info(`Answer Type: ${JSON.stringify(forecast.message)}`);
                }
            } catch (e) {
                console.warn(`Get forcast data - Error: ${e}`);
            }
        });
